...
<head>
    ...
    <fmt:message bundle="${loc}" key="locale.message.no.course" var="noCourse"/>
...
<div class="container">
    ...
    <c:if test="${trainingList.size() eq 0}">
        <div class="row">
            <div class="col-md-12">
                <h5><c:out value="${noCourse}"/></h5>
            </div>
        </div>
    </c:if>
    <c:if test="${trainingList.size() ne 0}">
        <div class="row">
            <div class="col-md-12">
                    ...
                    <c:forEach items="${trainingList}" var="training" varStatus="trainingLoopCout">
                        <tr>
                            ...
                            <c:if test="${training.courseStatus eq 'IN_PROCESS'}">
                                <td><span class="badge badge-success"><c:out value="${training.courseStatus}"/></span>
                                </td>
                            </c:if>
                            ...
                            <c:if test="${training.courseStatus eq 'REQUESTED'}">
                                <td><span class="badge badge-warning"><c:out value="${training.courseStatus}"/></span>
                                </td>
                            </c:if>
                            <c:if test="${training.courseStatus eq 'EXCLUDED' or training.courseStatus eq 'LEAVED' or training.courseStatus eq 'REJECTED'}">
                                <td><span class="badge badge-danger"><c:out value="${training.courseStatus}"/></span>
                                </td>
                            </c:if>
